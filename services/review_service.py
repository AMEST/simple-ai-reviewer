...
class ReviewService:
    ...
    def review_pull_request(self, diff: str) -> list[str]:
        results = []
        ...
        for diff_slice in splited_diff:
            ...
            prompt = self.__en_prompt(diff_slice) if self.configuration.language == Language.EN else self.__ru_prompt(diff_slice)
...
Ответ оформи в виде списка с метками: ✅ Плюсы,⚠️ Проблемы,💡 Советы
"""
    
    def __en_prompt(self, diff) -> str:
...
Format your answer as a list with tags: ✅ Pros, ⚠️ Problems, 💡 Tips
"""